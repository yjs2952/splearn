...
## 36강 애그리거트와 헥사고날 아키텍처

### 애그리거트

- DDD 에 소개된 도메인 모델 구성 요소/페턴의 하나
- 데이터 변경의 목적을 위해 하나의 단위로 취급되는 연관된 객체들의 클러스터(묶음)
- 루트(root)와 경계(boundary)를 가진다
- 데이터 변경 시 **하나의 단위**로 취급: 데이터 변경의 일관성 유지
- 루트를 통한 접근제어: 외부 객체는 **루트 엔티티에만 참조**를 가진다
- **데이터 일관성** 유지
    - 애그리거트를 넘어서는 불변식은 이벤트나 배치 등을 통해 특정 시간 내에 해결할 수 있다.
- 검색 및 접근 방식
    - **리포지토리**를 통해서 **애그리거트 루트만** 직접 얻을 수 있다
    - 내부 엔티티는 루트로부터 연관관를 통해서 접근한다
- 생명주기 관리 캡슐화
    - Factory, Repository 를 이용해서 객체들을 생명주기에 설쳐 체계적으로 의미있는 단위로 조작

### 애그리거트의 목표

- 일관성 유지
    - 객체 그룹에 적용되는 **불변식을 유지**하는 수단
- 이해 용이성
    - 객체의 시작과 끝을 명확히 한다
- 트랜잭션 및 동시성 관리
    - 트랜잭션 범위와 데이터 일관성 유지 방법 제공
- 모델 단순화
    - 연관 관계를 탐색을 제한하 루트를 통해서만 접근 하도록 함

### 애그리거트 적용 방법

- JPA cascading 을 적절하게 활용
    - root 가 수정되면 boundary 도 함께 수정
- repository 는 **애그리거트 단위**로 만든단
- 애그리거트 루트는 **엔티티**로 구현
...
- 적절한 애그리거트 경계를 선택하는게 어렵다
- 개발하면서 애그리거트 범위가 달라진다
    - 대체로 작은 애그리거트가 될 가능성이 높음
- 성능에 부담을 줌 (lazy loading의 도움이 필요)
- 내부 엔티티로의 직접 접근, 여러 애그리거트를 한번에 조회하는 기능이 필요할 때가 있다
- 도메인 이벤트, 최종적 일관성(eventual consistency)의 사용이 요구된다

### 헥사고널 아키텍처와 애그리거트

- 애그리거트 단위로 애플리케이션(헥사곤)을 구성 하는 방법이 유용하다
- 다른 애그리거트의 접근은 애플리케이션 포트를 통해서 ID를 전달하는 방식으로
    - 애플리케이션 내부 리포지토리에서 루트 엔티티를 조회하는 방식으로 이루어지게 강제할 수 있다.

## 37강 Member 애그리거트 개발

- 루트 애그리거트를 통해서만 바운더리를 제어할 수 있어야 하는데....
    - 코틀린에서는 패키지 접근 제어자가 없는데 어떻게 하지?
    - 다른 모듈로 보내야 하나?
